  ...
  fs.readFile(RANKING_FILE, "utf8", (err, data) => {
    if (err) {
      ...
      return res.status(500).json({
        error: "Hubo un error interno del servidor al leer el ranking.",
      });
    }
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Error interno al analizar los datos del ranking." });
    }
  });
});
  ...
  fs.readFile(RANKING_FILE, "utf8", (err, data) => {
    ...
    fs.writeFile(
      ...
      (writeErr) => {
        ...
        console.log(
          `Juego guardado: ${nuevaPartida.jugador}, ${nuevaPartida.puntaje} pts`
        );
